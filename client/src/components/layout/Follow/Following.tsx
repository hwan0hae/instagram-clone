...
function Following({ id, setModalVisible }: IFollowing) {
  const user = useRecoilValue(userAtom);
  const navigate = useNavigate();
  const isMutating = useIsMutating();
  const queryClient = useQueryClient();
  const ModalRef = useRef<HTMLDivElement>(null);
  const { data, isLoading } = useQuery<IGetFollowing>("following", () =>
    getFollowing(id)
  );
        ...
        <Overlay animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <Modal ref={ModalRef} initial={{ scale: 1.2 }} animate={{ scale: 1 }}>
            <TitleContainer>
              ...
              <Close>
                <SvgBtn onClick={() => setModalVisible(null)}>
