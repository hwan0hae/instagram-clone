...
- api호출시 post요청 데이터 params 사용 하던것 body에 데이터로 날리도록 수정
- 댓글 삭제 구현
- 게시물 삭제 구현
...
- api처리 전 중복 호출 방지 처리 (useIsMutating)
- MyPage 피드 hover시 댓글 및 좋아요 갯수 출력
- 좋아요시 하트 깜박임 이슈 수정 (렌더링시 기본값과 check 값 변경 되던것 수정)

Server

- 게시물 삭제시 서버에 저장된 사진 삭제
...
- user 프로필 없는ID의 경우 NotFound 페이지 출력

Client

- 페이지 이동시(라우터이동시) 스크롤(0,0)
- 라우터 이동시 모달(상세페이지 Detail)에도 스크롤(0,0) 되는 이슈 > 모달 닫힐시 location에 state scroll 값 주어 location state에 background || scroll 없을시에만 (0,0) 가도록 구현
- 라우터 /\* 없는 경로시 NotFound 페이지로 이동
- 피드 상세페이지에서 삭제시 백그라운드 주소로 이동 (삭제되어 피드 못불러오던 이슈) 해결
- 게시물 업로드시 홈, 상세페이지에서 사진의 크기에따라 깨짐 현상 수정
- 피드, 댓글 시간표시(경과) 구현
...
- 팔로우 기능 / 팔로워,팔로잉 리스트 / 홈 팔로잉 피드 출력

Client

- Submit Btn (로그인,가입,수정) 유효성 검사하여 버튼 활성화
- 가입시 ID 중복시 출력 수정 (이슈)
- 피드 상세페이지 createDate 추가
- likeList 팔로우 추가 / meatball 팔로우 취소
- filter > includes 사용
...
- 홈 Feed 무한 스크롤 기능 10장씩

Client

- Modal 출력시 헤더 올라가는 이슈
- 반응형 수정
